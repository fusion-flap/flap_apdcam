...
bool syncADCs(CAPDClient *client, int numADC, int * addresses)
{
	 ...
	 {
		 if (initRes)
			 initRes &= ReadPDI(client, addresses[i], ADC_REG_CONTROL, &controlReg, 1);
		 if (initRes)
		 {
	  		 controlReg |= 0x04;
			 initRes &= WritePDI(client, addresses[i], ADC_REG_CONTROL, &controlReg, 1);
		 }	
		 if (initRes)
		 {
			 usleep(1000);
...
bool setAllOffset(CAPDClient *  client, int numADC, int * addresses, unsigned int offset)
{
	...
	for (int i=0; i<32; i++)
	{
		buf[i*2] =  offset & 0x00ff;
		buf[i*2+1] = (offset >> 8) & 0xff;
	}
	for (int i = 0; i < numADC; ++i)
	{
		 if (initRes)
		 {
			 initRes &= WritePDI(client, addresses[i], ADC_REG_OFFSET, buf, 64);
		 }	
	}
	return initRes;
}
